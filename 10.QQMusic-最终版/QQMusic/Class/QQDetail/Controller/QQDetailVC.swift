...
extension QQDetailVC{
    ...
    override func viewDidLoad() {
        ...
        /** 注意：以上涉及到控件布局，都在另外一个方法中 ViewWillLayoutSubViews */
//        //打开摇一摇功能
...
extension QQDetailVC {
    ...
    func setSlide(){
        ...
        //值改变的时候
        progressSlider.addTarget(self, action: #selector(QQDetailVC.valueChange), for: .valueChanged)
        
        //添加手势
        progressSlider.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(tapGusture(sender:))))
    ...
    @objc func tapGusture(sender:UITapGestureRecognizer) -> () {
